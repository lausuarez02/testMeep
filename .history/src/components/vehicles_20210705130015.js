...
<tbody>
  {filteredVehicles.map(vehicle => {
    return (
      ...
      <td > <span className={vehicle.price_change_percentage_24h > 0 ? ("text-success") : ("text-danger")}>{formatPercent(coin.price_change_percentage_24h)}</span></td>
      <td style={{color:"white"}}>{formatDollar(vehicle.current_price, 20)}</td>
      <td style={{color:"white"}}>{formatDollar(vehicle.market_cap,12)}</td>
    </tr>
    )
  })}
   </tbody>
