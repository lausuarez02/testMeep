...
const Vehicles = () => {
  ...
  //Filter Vehicles
    const filteredVehicles = state.filter(vehicle =>
      vehicle.model.includes(search.toLowerCase()) || vehicle.licencePlate.toLowerCase().includes(search.toLowerCase())
    );
...
<tbody>
  {filteredVehicles.map(vehicle => {
    return (
      <tr key={vehicle.id}>
      <td style={{color:"white"}}> {vehicle.licencePlate}</td>
      <td  style={{color:"white"}}> {vehicle.x} {vehicle.y}</td>
